...
const blank = { code: '', name: '', email: '', phone: '', address: '', tax_id: '', payment_terms: '' }
...
export default function SuppliersPage() {
  ...
  async function load() {
    setLoading(true)
    try { setRows((await api.get('/suppliers')).data.data || []) }
  ...
  function openEdit(row) {
    setEditing(row)
    setForm({ code: row.code, name: row.name, email: row.email || '', phone: row.phone || '', address: row.address || '', tax_id: row.tax_id || '', payment_terms: row.payment_terms || '' })
    ...
    <div className="page">
      <div className="page-header">
        <div>
          ...
          <div className="page-desc">Vendor contacts and payment terms</div>
        </div>
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                <tr key={row.id}>
                  ...
                  <td style={{ color: '#888' }}>{row.payment_terms || '—'}</td>
      ...
      {showModal && (
        <Modal title={editing ? 'Edit Supplier' : 'New Supplier'} onClose={() => setShowModal(false)}>
          <form onSubmit={handleSubmit}>
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <input className="form-input" value={form.payment_terms} onChange={e => setForm({ ...form, payment_terms: e.target.value })} placeholder="e.g. Net 30" />
              </div>
            </div>
